...
public final class MyFindManagerImpl extends FindManager {
    private static final Logger LOG = Logger.getInstance(MyFindManagerImpl.class);
    ...
    private static final Key<Boolean> HIGHLIGHTER_WAS_NOT_FOUND_KEY = Key.create("MyFindManagerImpl.HighlighterNotFoundKey");
    ...
    public MyFindManagerImpl(@NotNull Project project) {
        myProject = project;
    ...
    public void showFindDialog(@NotNull FindModel model, @NotNull Runnable okHandler) {
        if (myHelper == null || Disposer.isDisposed(myHelper)) {
            myHelper = new MyFindUIHelper(myProject, model, okHandler);
    ...
    private static class FindExceptCommentsOrLiteralsData implements Predicate<FindResult> {
        ...
        static MyFindManagerImpl.FindExceptCommentsOrLiteralsData create(@NotNull VirtualFile file,
                                                                                              @NotNull FindModel model,
                                                                                              @NotNull CharSequence text,
                                                                                              @NotNull MyFindManagerImpl manager) {
            ...
            return new MyFindManagerImpl.FindExceptCommentsOrLiteralsData(file, model.clone(), ImmutableCharSequence.asImmutable(text), skipRangesSet);
        }
    ...
    private static final Key<ThreadLocal<SoftReference<MyFindManagerImpl.FindExceptCommentsOrLiteralsData>>> ourExceptCommentsOrLiteralsDataKey
    ...
    private Predicate<FindResult> getFindContextPredicate(@NotNull FindModel model, @Nullable VirtualFile file, @NotNull CharSequence text) {
        ...
        ThreadLocal<SoftReference<MyFindManagerImpl.FindExceptCommentsOrLiteralsData>> data;
        synchronized (model) {
            data = model.getUserData(ourExceptCommentsOrLiteralsDataKey);
            assert data != null;
        }

        SoftReference<MyFindManagerImpl.FindExceptCommentsOrLiteralsData> currentThreadDataRef = data.get();
        MyFindManagerImpl.FindExceptCommentsOrLiteralsData currentThreadData = currentThreadDataRef == null ? null : currentThreadDataRef.get();
        if (currentThreadData == null || !currentThreadData.isAcceptableFor(model, file, text)) {
            data.set(new SoftReference<>(currentThreadData = MyFindManagerImpl.FindExceptCommentsOrLiteralsData.create(file, model, text, this)));
        }
    ...
    private static final Key<ThreadLocal<SoftReference<MyFindManagerImpl.CommentsLiteralsSearchData>>> ourCommentsLiteralsSearchDataKey
    ...
    private FindResult findInCommentsAndLiterals(@NotNull CharSequence text,
        ...
        ThreadLocal<SoftReference<MyFindManagerImpl.CommentsLiteralsSearchData>> data;
        synchronized (model) {
        ...
        SoftReference<MyFindManagerImpl.CommentsLiteralsSearchData> currentThreadDataRef = data.get();
        MyFindManagerImpl.CommentsLiteralsSearchData currentThreadData = currentThreadDataRef == null ? null : currentThreadDataRef.get();
        if (currentThreadData == null || !Comparing.equal(currentThreadData.lastFile, file) || !currentThreadData.model.equals(model)) {
            ...
            try {
                ...
                currentThreadData =
                        new MyFindManagerImpl.CommentsLiteralsSearchData(file, relevantLanguages, highlighterAdapter, tokensOfInterest, searcher, matcher, model.clone());
